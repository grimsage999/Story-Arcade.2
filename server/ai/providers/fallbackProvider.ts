...
export class FallbackProvider implements AIProvider {
  ...
  async generateStory(input: AIStoryInput): Promise<AIStoryOutput> {
    ...
    storyLogger.info("Using fallback story generation");
    return story;
  }

  async generatePoster(input: AIPosterInput): Promise<AIPosterOutput | null> {
    // Return null to indicate that this provider doesn't handle poster generation
    return null;
  }

  async isHealthy(): Promise<boolean> {
    // Fallback provider is always healthy since it's local
    return true;
  }
